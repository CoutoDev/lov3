...
function startGame() {
    ...
    generateIcons().then(svgIcons => {
        itemTypes.push(
            ...
            { type: 'lilo', icon: svgIcons.lilo, points: 3, weight: 0.15 },
            { type: 'ze', icon: svgIcons.ze, points: 3, weight: 0.15 },
            { type: 'joyPhoto', icon: svgIcons.joyPhoto, points: 10, weight: 0.11 },
            { type: 'joyTrip', icon: svgIcons.joyTrip, points: 15, weight: 0.19 },
            { type: 'special', icon: svgIcons.special, points: 20, weight: 0.17 }
        )
        resizeCanvas();
        animationId = requestAnimationFrame(update);
    })
